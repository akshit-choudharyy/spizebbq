    ...
    <div className='flex flex-col sm:flex-row flex-wrap gap-12 justify-center items-center mt-2'>
      {categoryItems.map((item) => (
        <a key={item.id} href="#" className="group relative block w-full sm:w-[48%] lg:w-[22%]">
          <div className="relative h-[350px] sm:h-[450px] overflow-hidden rounded-md">
            <img
              src={item.image}
              alt={item.title}
              className="absolute inset-0 h-full w-full object-cover opacity-100 group-hover:opacity-0 transition-opacity duration-300 rounded-md"
            />
            {item.image2 && (
              <img
                src={item.image2}
                alt={item.title}
                className="absolute inset-0 h-full w-full object-cover opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-md"
              />
            )}
          </div>
          <div className="absolute inset-0 flex flex-col items-start justify-end p-6 bg-gradient-to-t from-black to-transparent rounded-md">
            <h3 className="text-xl font-medium text-white">{item.title}</h3>
            <p className="mt-1.5 text-pretty text-xs text-white">
              {item.desc}
            </p>
            <a href="/menu">
              <span className="btn rounded-xl flex items-center gap-2 px-6 m-2" style={{ backgroundColor: '#FF6500', color: '#ffffff' }}>
                View Menu
              </span>
            </a>
          </div>
